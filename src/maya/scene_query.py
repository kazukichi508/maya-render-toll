...
def _get_node_type(node_path: str):
    """
    transform 配下の shape を見てタイプ/PV を判定
    戻り値: {'type': 'geometry'|'light'|'camera'|'group'|'other', 'primaryVisibility': bool|None}
    """
    _ensure_mtoa_loaded()
    ...
    if not is_light:
        if cmds.nodeType(shp) in ("aiAreaLight", "aiSkyDomeLight", "aiMeshLight", "aiPhotometricLight"):
            is_light = True
    if is_light:
        info["type"] = "light"
        return info
    ...
    if cmds.attributeQuery("primaryVisibility", node=shp, exists=True):
        try:
            info["primaryVisibility"] = cmds.getAttr(f"{shp}.primaryVisibility")
        except Exception:
            pass
    return info
...
def get_renderable_descendants(root_node: str):
    """root_node 配下のレンダリング可能 transform を列挙（camera/light 除外）"""
    out = set()
...
def get_all_renderable_shapes_in_scene():
    ...
    logger.debug("Calculating all renderable shapes in scene...")
    _ensure_mtoa_loaded()
    out = []
    cache = {}
    for shp in cmds.ls(type="shape", long=True, noIntermediate=True) or []:
        try:
            ...
            if nt not in cache:
                try:
                    cache[nt] = cmds.nodeType(nt, inherited=True) or []
                except Exception:
                    cache[nt] = []
            inh = cache[nt]
            if "camera" in inh or "light" in inh or "aiLight" in inh:
            ...
            logger.debug(f"Skip {shp}: {e}")
    logger.debug(f"Found {len(out)} renderable shapes.")
    return out
